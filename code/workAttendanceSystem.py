...
class WAS(wx.Frame):
            ...
            if len(dets) != 0:
                ...
                for i, knew_face_feature in enumerate(self.knew_face_feature):
                    ...
                    if compare == "same":  # 找到了相似脸
                        ...
                        if nowdt[nowdt.index(" ")+1:-1] <= self.puncard_time or True:
                            self.infoText.AppendText(nowdt + "工号:" + str(self.knew_id[i])
    ...
    def initDatabase(self):
        ...
        face_feature blob not null) engine = innodb default charset = utf8''')
        cur.execute('''create table if not exists logcat
         (datetime text not null,
         id int not null,
         name text not null,
         late text not null) engine = innodb default charset = utf8''')
        cur.close()
        conn.commit()
        conn.close()
    ...
    def insertARow(self,Row,type):
        ...
        cur = conn.cursor()  # 得到游标对象
        if type == 1:
            cur.execute("insert into student_info (id,name,face_feature) values(%s,%s,%s)",
    ...
    def loadDataBase(self,type):
        ...
        if type == 1:
            ...
            cur.execute('select id,name,face_feature from student_info')
            origin = cur.fetchall()
            for row in origin:
                print(row[0])
                ...
                print('row[2]: ',self.convert_array(row[2]).shape)
